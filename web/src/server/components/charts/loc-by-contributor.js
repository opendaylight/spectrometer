...
export default class LocByContributorChart extends Component {
  ...
  render() {
          ...
          >
            {dataSeries.map((project, index) => (
              <TableRow>
                ...
                <TableRowColumn>{project.loc}</TableRowColumn>
               </TableRow>
            ))}
          </TableBody>
        </Table>
    ...
    if (!_.isEmpty(this.props.projects)) {
      dataSeries = DataReducers.authorsVsLocForAllProjects(this.props.projects, this.state.view.sortBy)
    }
    let fullData = dataSeries
    dataSeries = DataReducers.sliceAndGroupOthers(dataSeries.reverse(), 12, 'loc')
    console.log("contributor-vs-loc-chart:render", this.props.organization, this.props.author)

    return (
      <PaperLayout id="projects-vs-loc" zDepth={2} title="Lines of Code by Contributor"
...
LocByContributorChart.propTypes = {
